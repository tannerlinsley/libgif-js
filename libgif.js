...
(function(root, factory) {
    ...
    var parseGIF = function(st, handler, first, proceed) {
        handler || (handler = {});
        ...
        var parseBlock = function() {
            ...
            if (proceed) {
                proceed.forth = function(cb) {
                    callback = cb;
                    setTimeout(parseBlock, 0);
                };
            }

            if (first && block.type == 'img') {
                first = false;
            } else if (block.type !== 'eof') {
                setTimeout(parseBlock, 0);
            } else {
                if (proceed.done) {
                    proceed.done.call();
    ...
    var SuperGif = function(opts) {
        ...
        var doParse = function(first, proceed) {
            try {
                parseGIF(stream, handler, first, proceed);
            } catch (err) {
                doLoadError('parse');
            }
        };
        ...
        return {
            ...
            load_url: function(src, callback, first, proceed) {
            ...
            loadFirst: function(callback) {
                var proceed = proceed || {};
                this.load_url(gif.getAttribute('rel:animated_src') || gif.src, callback, true, proceed);
                return proceed;
            },
